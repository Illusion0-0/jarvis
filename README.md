...
> Your voice, your memory, your agent. Runs 24/7 on a free cloud VM.

JARVIS is a personal clone: an always-on agent whose job is being you, digitally. It speaks with your cloned voice and remembers what you teach it, because its brain is your own knowledge vault, git-synced to the agent. A tiered LLM setup keeps it cheap by default and smart on demand. It also acts while you sleep: scheduled jobs, watchers, research, reporting, publishing, without you opening a laptop.

One running instance costs about $0/month on a free-tier cloud VM. This repo is the blueprint: architecture, design principles, and the full use-case surface of a working reference implementation that has been live since September 2026.
...
|---|---|---|
| 1 | **You are the unit** | Identity, memory, and voice belong to one owner and persist across every chat. The deployment models a person, not a conversation. |
| 2 | **Brain before mouth** | Memory is a first-class subsystem: a git-synced second brain the agent reads and writes back to. Answers cite the brain; new durable facts get distilled into it. The clone compounds. |
...
| 4 | **Multimodal in and out** | Anything you can send (text, images, video, audio, PDFs, URLs), it can ingest. Replies come back as text, voice notes, rendered dashboards, or documents. |
| 5 | **Automation-first** | If a task is recurring, it should never be requested twice. Cron jobs, watchers, and digest builders ship as core subsystems. |
...
| 7 | **It must survive you closing the laptop** | The agent runs as a systemd service on a server, so your laptop being shut changes nothing. Reboots, sleeps and travel do not reach it. 24/7 is the baseline. |
...
This is the catalog for one deployment. Most of it runs today in the reference implementation; the rest are direct extensions of the same primitives.

### Daily operating system
- **Morning briefing**: weather, agenda, priorities from your goal notes, overnight watcher results, in one voice note at a fixed hour.
...
### Home ops
- **Household watchdogs**: anything with a page or a feed: order status, appointment slots, bill due dates, service reminders.
...
| Voice | [Pocket-TTS](https://kyutai.org/) (100M params, CPU) | cloned speech at RTF 1.37x measured on 2 ARM cores, no GPU |
